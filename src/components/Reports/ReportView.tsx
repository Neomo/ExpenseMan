...
export const ReportView: React.FC = () => {
  ...
  // Available categories for multi-select filtering
  const allTransportOptions = useMemo(() => {
    const defaults = ['高铁', '的士', '网约车', '大巴', '火车', '飞机'];
    const presentInTrips = Array.from(new Set(trips.map((t) => t.transport).filter(Boolean)));
    return Array.from(new Set([...defaults, ...presentInTrips]));
  }, [trips]);

  const allExpenseCategoryOptions = useMemo(() => {
    const defaults = ['交通', '餐饮', '住宿', '物品', '饮品', '水果', '礼物', '补贴', '其他'];
    const presentInExpenses = Array.from(new Set(expenses.map((e) => e.category).filter(Boolean)));
    return Array.from(new Set([...defaults, ...presentInExpenses]));
  }, [expenses]);

  // Apply Category Filters to Trips and Expenses
  const displayTrips = useMemo(() => {
    if (selectedTransports.length === 0) return filteredTrips;
    return filteredTrips.filter((t) => selectedTransports.includes(t.transport));
  }, [filteredTrips, selectedTransports]);

  const displayExpenses = useMemo(() => {
    if (selectedExpenseCategories.length === 0) return filteredExpenses;
    return filteredExpenses.filter((e) => selectedExpenseCategories.includes(e.category));
  }, [filteredExpenses, selectedExpenseCategories]);
  ...
  const toggleExpenseCategoryFilter = (cat: string) => {
  ...
  // Totals
  const tripSubtotal = displayTrips.reduce((sum, t) => sum + t.amount, 0);
  const expenseSubtotal = displayExpenses.reduce((sum, e) => sum + e.amount, 0);
          ...
          <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
            <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-3 flex items-center justify-between">
              <span className="flex items-center gap-2">
                ...
                行程明细表 ({displayTrips.length} / 共 {filteredTrips.length} 条)
              </span>
              <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">
                当前筛选小计: ¥ {tripSubtotal.toFixed(2)}
              </span>
            </h3>

            {/* Filter Bar for Transport Categories */}
            <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/80 flex flex-wrap items-center gap-2 text-xs">
              ...
              >
                全部类别 ({filteredTrips.length})
              </button>

              {allTransportOptions.map((opt) => {
                const count = filteredTrips.filter((t) => t.transport === opt).length;
                const isSelected = selectedTransports.includes(opt);
                return (
                  <button
                    key={`transport-filter-${opt}`}
                    type="button"
                    onClick={() => toggleTransportFilter(opt)}
                  ...
                  >
                    <span>{opt}</span>
                    <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${isSelected ? 'bg-blue-500 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-500'}`}>
                      {count}
                    </span>
                  </button>
                );
            ...
            {displayTrips.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">选定类别或时间区间内无相关行程记录</p>
            ) : (
          ...
          <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
            <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-3 flex items-center justify-between">
              <span className="flex items-center gap-2">
                ...
                其他费用明细表 ({displayExpenses.length} / 共 {filteredExpenses.length} 条)
              </span>
              <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                当前筛选小计: ¥ {expenseSubtotal.toFixed(2)}
              </span>
            </h3>

            {/* Filter Bar for Expense Categories */}
            <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/80 flex flex-wrap items-center gap-2 text-xs">
              <span className="font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5 shrink-0">
              ...
              >
                全部类别 ({filteredExpenses.length})
              </button>

              {allExpenseCategoryOptions.map((opt) => {
                const count = filteredExpenses.filter((e) => e.category === opt).length;
                const isSelected = selectedExpenseCategories.includes(opt);
                return (
                  <button
                    key={`exp-filter-${opt}`}
                    type="button"
                    onClick={() => toggleExpenseCategoryFilter(opt)}
                    className={`px-2.5 py-1 rounded-lg font-bold transition-all text-xs flex items-center gap-1 ${
                      isSelected
                        ? 'bg-emerald-600 text-white shadow-xs'
                        : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'
                  ...
                  >
                    <span>{opt}</span>
                    <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${isSelected ? 'bg-emerald-500 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-500'}`}>
                      {count}
                    </span>
                  </button>
                );
            ...
            {displayExpenses.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">选定类别或时间区间内无其他费用记录</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-left text-slate-700 dark:text-slate-300">
                  <thead className="bg-slate-50 dark:bg-slate-800/60 font-semibold text-slate-900 dark:text-slate-100 border-b border-slate-200 dark:border-slate-800">
